...
ABLATIONS = ["", "no_curriculum", "no_length", "sft"] 
# ABLATIONS = ["sft"] 
...
def worker(gpu_queue, job_queue):
    while not job_queue.empty():
        ...
        val_file = os.path.join(DATA_ROOT, dataset, "test.jsonl") # 或者是 validation.jsonl，根据您实际情况
        
        # 兼容性检查：如果 test.jsonl 不存在，尝试找 test.json
        ...
        try:
            ...
            if method == "sft":
                ...
                cmd = (
                    ...
                    f"--train_file {train_file} "  # 使用 explicit file
                    f"--val_file {val_file} "      # 使用 explicit file
                    f"--run_name {run_name} "
                    f"--loss_type sft "
                ...
                cmd_gen = (
                    ...
                    f"--base_model {family['teacher_base']} "
                    f"--model_path {teacher_ckpt} "
                    f"--input_file {train_file} " # 使用 explicit file
                ...
                cmd_train = (
                    ...
                    f"--teacher_model {teacher_ckpt} "
                    f"--student_model {family['student']} "
                    f"--train_file {gen_file} "    # Explicit generated file
                    f"--val_file {val_file} "      # Explicit val file
                    f"--run_name {run_name} "
                    f"--loss_type sft "
                    f"--student_mode supervised "
                    f"--num_epochs {epoch} "
                ...
                cmd = (
                    ...
                    f"-b {family['student']} "
                    f"-t {teacher_ckpt} "
                    f"-f {train_file} " # 传入 explicit train file
                    f"-v {val_file} "   # 传入 explicit val file
                    f"-n {run_id}" 
                )
                with open(log_file, "a") as f:
